...
export default function Profile() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <PageHead title="My Profile" noindex />
      <Header />
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-soft animate-fade-in">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <Input id="username" placeholder="Enter your username" value={username} onChange={e => setUsername(e.target.value)} className="bg-background" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="avatarUrl" type="url" placeholder="https://example.com/avatar.jpg" value={avatarUrl} onChange={e => setAvatarUrl(e.target.value)} className="bg-background" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="bio">Bio</Label>
              <Textarea id="bio" placeholder="Tell us a bit about yourself..." value={bio} onChange={e => setBio(e.target.value)} className="bg-background min-h-[120px] resize-none" maxLength={500} />
        ...
        <div className="mt-6 bg-card border border-border rounded-xl overflow-hidden shadow-soft">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            <span className="text-sm text-muted-foreground">{myReviews.length} total</span>
          </div>

          {myReviews.length === 0 ? (
            <div className="px-6 py-10 text-center text-muted-foreground text-sm">
              You haven't written any reviews yet.{" "}
              <Link to="/write-review" className="text-primary hover:underline">Write one</Link>
            </div>
          ) : (
                ...
                <li key={rv.id}>
                  <Link to={`/review/${rv.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-muted/30 transition-colors group">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{rv.product} · {new Date(rv.created_at).toLocaleDateString()}</p>
                    </div>
        ...
        <div className="mt-6 mb-8 bg-card border border-border rounded-xl overflow-hidden shadow-soft">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            <span className="text-sm text-muted-foreground">{myComments.length} total</span>
          </div>
                  ...
                  <Link to={`/review/${cm.review_id}`} className="flex items-start gap-4 px-6 py-4 hover:bg-muted/30 transition-colors group">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span>·</span>
                        <span className="truncate">on <span className="font-medium text-foreground/70 group-hover:text-primary transition-colors">{cm.review_title}</span></span>
                        <span>·</span>
